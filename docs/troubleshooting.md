...
- **"--password is required"** — the restic password is the encryption key for your backup repo (a root recovery secret: snapshots hold every service secret). Prefer `backup setup --generate`. After setup it is escrowed in your vault (`backup recovery-kit <base>` reprints it) and printed in the recovery kit — if the vault is gone too, you need the offline copy from setup/rekey time.
