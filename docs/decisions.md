...
## Local development (`ownbasectl dev`)

| Decision | Choice | Why |
|---|---|---|
| Agent-safe / human-run split | `create`/`vm start\|stop\|restart` never prompt for anything, ever, under any circumstance; the only interactive command anywhere in `ownbasectl` is `dev <name>`, which may prompt for `sudo` exactly once (mkcert's CA install) | `create`/`vm` are expected to be run by an AI agent unattended — an agent cannot answer a TTY password prompt. Starting `dev` is itself a human's explicit "I am sitting here, ready to develop" signal, so an interactive prompt there is acceptable |
| No-domain Base exposure | A Base with no domain'd service exposes only SSH externally (Caddy publishes no ports, UFW opens no web ports) — this retires the old assumption that a no-domain Base gets direct, unproxied port access | There is nothing for Caddy to route to yet, and the dev bridge reaches services directly over SSH, so publishing ports pre-emptively only widens the attack surface for no benefit |
