...
| Tunnel port allocation | The compiler publishes each bridged service's container port to its own loopback port, `schema.DevBridgeBasePort` (41000) + sorted index among eligible services (`OwnbaseConfig.DevBridgePorts()`) — never the same number as the container port | Decoupling the host-side loopback port from the container's own `port:` means a service can declare `port: 80`/`443` (colliding with Caddy's machine-wide bind) or share a port number with another service, with no collision on either side; both the compiler (rendering the Quadlet unit) and `ownbasectl tunnel` (parsing `ownbase.yaml` independently, no daemon call) compute this from the same config and always agree without coordinating |
