...
| Transport | Direct SSH tunnels to each bridged service's container port (`internal/tunnel`, reused unmodified), bypassing Caddy entirely | No port needs to be published or firewalled on the Base at all; works identically for a local VM or a remote server since both use the same `ServerProfile` |
| Bridge scope | Only services with both a `port:` and at least one domain configured are bridged; a domain-less service is never bridged | Mirrors exactly what is already intentionally publicly reachable in production — never invents local-only access to something the operator hasn't chosen to expose via a domain |
