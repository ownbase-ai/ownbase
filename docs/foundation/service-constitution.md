...
OwnBase provides services — auth, jobs, storage, a Git host — so a user (or their AI) doesn't have to reinvent them per app. That convenience is also the exact place lock-in usually creeps in. This document is the wall against that.
...
The user can remove any service. Removing a service must not break the Base or hold the rest of their system hostage. If apps depend on the capability it provided, removal must fail loudly and clearly — never silently corrupt — but the user always has the power to take it out.
...
**The user owns:** the server account, all app code, all service code, all data, all configuration, all secrets, all backups, all domains.
...
Because apps reference the capability (`auth`) rather than the product (`ownbase-auth`), a user can move from:
