...
- **Per-service containers, least privilege.** Each service runs in its own container with **every Linux capability dropped by default** — a service that needs one back must declare it (`add_capabilities:`), which is why binding a privileged port is an explicit, visible decision. No privileged daemon, and no service mounts volumes from another service. The one intentional host bind is the **per-service daemon API socket** when a service opts in with `ownbase_access:` — a scoped unix socket, not the Podman runtime socket, and only for services that declare it.
- **Scoped secrets.** Each service receives only the secrets it is declared to need **by injection** (its own age file). Secrets are age-encrypted and injected at start — see [ownbase-yaml.md](../ownbase-yaml.md), "Secrets". Cross-service secret access is possible only when the owner grants an explicit `secrets:<svc>:read` (or `*`) scope in `ownbase_access`; it is never the default.
- **Internal network segmentation.** Services reach declared capabilities (auth, jobs, database) over per-capability networks only; they cannot reach the host runtime, the daemon, or other services' private ports **by default**. A service with `ownbase_access` reaches the daemon over its private socket under the scopes the owner granted.
...
Every action the daemon takes is drawn from a closed taxonomy (`internal/schema/taxonomy.go`) and pre-classified into a risk tier — **autonomous** (act immediately), **notify** (act and tell the owner), or **approve** (suspend until approved). Tiers already have teeth for service principals: `TierApprove` actions are refused outright, and host-mutating routes (token reset, self-update, config source, …) are owner-only even when a service is granted `*`. An external approval device for owner `approve` tiers is still post-V1; until then the owner path acts and records. Every action is recorded in an audit log the user owns — with the **acting principal** (owner or `service:<name>`) — and can be exported or deleted at any time. An action type that isn't in the taxonomy cannot be executed.
