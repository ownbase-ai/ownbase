...
**OwnBase** : The system that turns a machine the customer controls into a secure, self-maintaining home for the software they own.

**a Base** : One customer's installation of OwnBase — a single owned machine, everything on it, and its config repo. "Create a Base," "the Base is healthy." Plural: Bases.
...
**App** : A piece of software the customer owns and runs on their Base. Apps depend on *capabilities*, not on specific service implementations.
...
**Drift** : A difference between what `ownbase.yaml` declares and what is actually running — reported by `ownbasectl updates`/`checkup`, never silently auto-corrected against the customer's `ref:` intent. Also used for a change in generated `runtime/` files that the compiler did not make (a tamper signal), since the compiler is the only thing that may write `runtime/`.
...
**`ownbase.yaml`** : The single control file in the customer's repo that defines core settings and services. The contract between `ownbasectl`, the daemon, and any AI. See [ownbase-yaml.md](../ownbase-yaml.md).
