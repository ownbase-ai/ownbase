...
func TestGrepPublishPortCommand_TargetsSystemQuadletDir(t *testing.T) {
	if bridge.SystemQuadletDir != "/etc/containers/systemd" {
		t.Fatalf("SystemQuadletDir = %q, want /etc/containers/systemd", bridge.SystemQuadletDir)
	}
	if !strings.Contains(bridge.GrepPublishPortCommand, bridge.SystemQuadletDir) {
		t.Errorf("GrepPublishPortCommand should reference SystemQuadletDir, got %q", bridge.GrepPublishPortCommand)
	}
}
